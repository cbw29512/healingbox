...
function playerName(value){return String(value||'Player').replace(/[\u0000-\u001f\u007f]/g,'').trim().slice(0,40)||'Player'}
...
function joinParty(){displayName=playerName($('#joinName').value);roomCode=$('#joinCode').value.trim().toUpperCase();if(roomCode.length!==6){toast('Enter the six-character room code.');return}setConnection('Connecting…');peer=new Peer();peer.on('open',()=>{hostConn=peer.connect(hostId(roomCode),{reliable:true});hostConn.on('open',()=>{hostConn.send({type:'hello',name:displayName});setConnection('Connected',true)});hostConn.on('data',data=>{if(data.type==='state'){state=data.state;render()}if(data.type==='error')toast(data.message)});hostConn.on('close',()=>setConnection('Host disconnected',false))});peer.on('error',error=>setConnection(`Connection error: ${error.type}`,false))}
function handleRemote(data,connection){if(data?.type==='hello'){connection.metadata={name:playerName(data.name)};connection.send({type:'state',state,roomCode});return}if(data?.type==='action'){const actor=connection.metadata?.name||'Player';applyAction(data.action,actor,connection,'remote')}}
function sendAction(action){if(isHost)applyAction(action,'DM',null,'host');else if(hostConn?.open)hostConn.send({type:'action',action});else toast('The DM host is not connected.')}

function applyAction(action,actor,connection,source){try{if(!action||typeof action.type!=='string')throw new Error('Invalid artifact action.');if((action.type==='rest'||action.type==='undo')&&source!=='host')throw new Error('Only the hosting DM can perform that action.');if(action.type==='heal')useHealingCharge(action.chargeId,actor);else if(action.type==='scroll')useScrollCharge(action,actor);else if(action.type==='rest'){state.charges=makeCharges(state.level);state.log=[];log('The party completed a long rest. Every divine charge returned.')}else if(action.type==='undo')undoLast();else throw new Error('Unknown artifact action.');broadcast()}catch(error){console.error(error);if(connection?.open)connection.send({type:'error',message:error.message});else toast(error.message)}}
